...
class SkillBasedDialogManager:
    ...
    def _extract_slot(self, step: Dict, user_message: str, state: SkillState) -> Optional[str]:
    ...
    def _run_handler(self, handler_name: str, state: SkillState) -> str:
        try:
            ...
            return "Ett tekniskt fel uppstod. Försök igen."
    ...
    async def respond(
        ...
        # ── Single-slot step ──────────────────────────────────────────────────
        if step.get("slot"):
            extracted = self._extract_slot(step, user_message, state)
